...
function loadVioScopePrompt() {
  const promptPath =
    [
      resolve(process.cwd(), 'src/mastra/agents/vioscope.prompt.yaml'),
      fileURLToPath(new URL('./vioscope.prompt.yaml', import.meta.url)),
    ].find((path) => existsSync(path)) || resolve(process.cwd(), 'src/mastra/agents/vioscope.prompt.yaml');
