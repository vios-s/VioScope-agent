...
| [~] | T7.6 | Config references real accounts. | `coordinator_user` and `member_users` use `users.username`. | External theme meeting config uses username fields; the current Theme B coordinator/member assignment was updated outside the public repo. DB lookup shows the referenced account rows exist but are still `profile_only`, so active-account enforcement remains incomplete. |
