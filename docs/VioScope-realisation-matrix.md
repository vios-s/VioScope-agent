...
| [x] | I1.5 | `DATASTORE_DIR` is configured. | Internal config/materials can be read from outside public code. | `DATASTORE_DIR=/Public` exists; theme meeting files, VIOS skill roots, upload roots, and GitBook sync state live outside public code. The old `lab-state.yaml` runtime path has been removed. |
...
| [x] | M3.3 | Public memory path is retired. | Project operations no longer depend on YAML-backed public lab state. | `npm run check:memory` covers session memory, personal-memory isolation, and shared-chat memory boundaries; project operations use Project Manager DB records. |
...
| Status | ID | Check | Expected behaviour | Evidence / notes |
|---|---|---|---|---|
| [x] | ST6.1 | Project state exists without YAML. | Operational state is stored in Postgres Project Manager records; the old `lab-state.yaml` API/tools/reader are removed. | `project_records`, `project_artifacts`, `project_updates`, and comments tables are implemented; `npm run check:projects` passes. |
